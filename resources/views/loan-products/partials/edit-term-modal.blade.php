<div class="modal fade" id="editTermsModal{{ $term->id }}" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form action="{{ route('loan-product-term.update', $term->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-6 mb-4">
                        <div class="col">
                            <label for="Interest_Rate" class="form-label">Interest Rate
                                <x-required /></label>
                            <input type="text" id="Interest_Rate" class="form-control" placeholder="Interest Rate"
                                name="Interest_Rate" value="{{ old('Interest_Rate', $term->Interest_Rate) }}">
                        </div>
                        <div class="col">
                            <label for="Interest_Charged_At" class="form-label">Interest Charged At
                                <x-required /></label>
                            <select id="Interest_Charged_At" class="form-control" name="Interest_Charged_At">
                                @foreach (\App\Models\LoanSchedule::INTEREST_CHARGED_AT as $item)
                                    <option value="{{ $item }}"
                                        {{ old('Interest_Charged_At', $term->Interest_Charged_At) == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
                            </select>

                            @error('Interest_Charged_At')
                        ...
                        <div class="col">
                            ...
                            <select id="Interest_Cycle" class="form-control" name="Interest_Cycle">
                                @foreach (\App\Models\LoanSchedule::INTEREST_CYCLES as $item)
                                    <option value="{{ $item }}"
                                        {{ old('Interest_Cycle', $term->Interest_Cycle) == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-4">
                        <div class="col">
                            ...
                            <select id="Interest_Calculation_Method" class="form-control"
                                ...
                                @foreach (\App\Models\LoanSchedule::SUPPORT_INTEREST_METHODS as $item)
                                    <option value="{{ $item }}"
                                        {{ old('Interest_Calculation_Method', $term->Interest_Calculation_Method) == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col">
                            ...
                            <input type="text" id="Term" class="form-control" placeholder="Maximum Term allowed"
                                name="Value" value="{{ old('Term', $term->Value) }}">
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="mb-4 form-check">
                            @php
                                $existingRepaymentCycles = json_decode($term->Repayment_Cycles, true); // Decode JSON into array
                            @endphp
                            @foreach (\App\Models\LoanSchedule::REPAYMENT_FREQUENCIES as $item)
                                <div id="Repayment_Cycles">
                                    <input type="checkbox" name="Repayment_Cycles[]" value="{{ $item }}"
                                        ...
                                        @if (in_array($item, $existingRepaymentCycles)) checked @endif>
                                    <label for="{{ $item }}"
                    ...
                    <div class="mb-4">
                        <input type="checkbox" class="form-check-input" id="Has_Advance_Payment"
                            ...
                            {{ old('Has_Advance_Payment', $term->Has_Advance_Payment) ? 'checked' : '' }}>
                    ...
                    <!-- Calculation Method -->
                    <div class="mb-4">
                        <label for="Advance_Calculation_Method" class="form-label">Advance Calculation Method
                            <x-required /></label>
                        <select id="Advance_Calculation_Method" class="form-control" name="Advance_Calculation_Method">
                            <option value="">Choose...</option>
                            <option value="Flat"
                                {{ old('Advance_Calculation_Method', $term->Advance_Calculation_Method) == 'Flat' ? 'selected' : '' }}>
                                Flat
                            </option>
                            <option value="Percentage"
                                {{ old('Advance_Calculation_Method', $term->Advance_Calculation_Method) == 'Percentage' ? 'selected' : '' }}>
                                Percentage
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" class="form-control" id="Advance_Value" name="Advance_Value"
                            value="{{ old('Advance_Value', $term->Advance_Value) }}">
                    </div>
                    ...
                    <div class="mb-4">
                        <input type="checkbox" class="form-check-input" id="Extend_Loan_After_Maturity"
                            ...
                            {{ old('Extend_Loan_After_Maturity', $term->Extend_Loan_After_Maturity) ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control" id="Interest_Type_After_Maturity"
                            ...
                            value="{{ old('Interest_Type_After_Maturity', $term->Interest_Type_After_Maturity) }}">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" class="form-control" step="0.01" id="Interest_Value_After_Maturity"
                            name="Interest_Value_After_Maturity"
                            value="{{ old('Interest_Value_After_Maturity', $term->Interest_Value_After_Maturity) }}">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control" id="Interest_After_Maturity_Calculation_Method"
                            ...
                            value="{{ old('Interest_After_Maturity_Calculation_Method', $term->Interest_After_Maturity_Calculation_Method) }}">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control" id="Recurring_Period_After_Maturity_Type"
                            ...
                            value="{{ old('Recurring_Period_After_Maturity_Type', $term->Recurring_Period_After_Maturity_Type) }}">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" class="form-control" id="Recurring_Period_After_Maturity_Value"
                            ...
                            value="{{ old('Recurring_Period_After_Maturity_Value', $term->Recurring_Period_After_Maturity_Value) }}">
                    </div>
                    ...
                    <div class="mb-4">
                        <input type="checkbox" class="form-check-input" id="Include_Fees_After_Maturity"
                            ...
                            {{ old('Include_Fees_After_Maturity', $term->Include_Fees_After_Maturity) ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" class="form-control" id="Write_Off_After_Days"
                            name="Write_Off_After_Days"
                            value="{{ old('Write_Off_After_Days', $term->Write_Off_After_Days) }}">
                    </div>
                </div>
